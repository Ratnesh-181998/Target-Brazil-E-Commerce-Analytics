-- ===================================================================
-- TARGET SQL CASE STUDY - COMPLETE SOLUTION
...
SELECT 
    ...
    CAST(COUNT(DISTINCT customer_id) * 100.0 / SUM(COUNT(DISTINCT customer_id)) OVER () AS DECIMAL(5,2)) AS percentage
FROM customers
...
SELECT 
    ...
    CAST(((SELECT total_cost FROM OrderCosts WHERE order_year = 2018) - 
          (SELECT total_cost FROM OrderCosts WHERE order_year = 2017)) * 100.0 / 
         (SELECT total_cost FROM OrderCosts WHERE order_year = 2017) AS DECIMAL(5,2)) AS percentage_increase;
...
SELECT 
    ...
    DATEDIFF(DAY, o.order_purchase_timestamp, o.order_delivered_customer_date) AS time_to_deliver,
    DATEDIFF(DAY, o.order_delivered_customer_date, o.order_estimated_delivery_date) AS diff_estimated_delivery
FROM orders o
...
SELECT TOP 5
    c.customer_state,
    AVG(CAST(DATEDIFF(DAY, o.order_purchase_timestamp, o.order_delivered_customer_date) AS FLOAT)) AS avg_delivery_time
FROM orders o
INNER JOIN customers c ON o.customer_id = c.customer_id
WHERE o.order_delivered_customer_date IS NOT NULL
GROUP BY c.customer_state
...
SELECT TOP 5
    c.customer_state,
    AVG(CAST(DATEDIFF(DAY, o.order_delivered_customer_date, o.order_estimated_delivery_date) AS FLOAT)) AS avg_days_faster_than_estimated
FROM orders o
INNER JOIN customers c ON o.customer_id = c.customer_id
WHERE o.order_delivered_customer_date IS NOT NULL
    AND o.order_estimated_delivery_date IS NOT NULL
GROUP BY c.customer_state
HAVING AVG(CAST(DATEDIFF(DAY, o.order_delivered_customer_date, o.order_estimated_delivery_date) AS FLOAT)) > 0
...
SELECT 
    ...
    CAST(COUNT(DISTINCT CASE WHEN order_count > 1 THEN customer_id END) * 100.0 / 
         COUNT(DISTINCT customer_id) AS DECIMAL(5,2)) AS retention_rate
FROM (
    SELECT 
        customer_id,
        COUNT(*) AS order_count
